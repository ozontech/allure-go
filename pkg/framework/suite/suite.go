...
func (s *Suite) RunInitParamsSuite(t provider.T, suite runner.TestSuite) runner.SuiteResult {
	t.SkipOnPrint()
	parts := strings.Split(t.RealT().Name(), "/")
	parentName := parts[len(parts)-3]
	return runner.NewInitParamsSuiteRunnerWithParent(t.RealT(), getPackage(2), cleanName(getSuiteName(suite)), parentName, suite).RunTests()
}
...
func RunInitParamsSuite(t provider.TestingT, suite runner.TestSuite) runner.SuiteResult {
	return runner.NewInitParamsSuiteRunner(t, getPackage(2), getSuiteName(suite), suite).RunTests()
}
